...
void makeSphere(float radius, int slices, int stacks, VtxOutIter vtxIter, IdxOutIter idxIter) {
   ...
   for (int i = 0; i < slices + 1; ++i) {
      for (int j = 0; j < stacks + 1; ++j) {
         float x = (float) (longCos[i] * latSin[j]);
         float y = (float) (longSin[i] * latSin[j]);
         float z = (float) latCos[j];

         Cvec3f n(x, y, z);
         Cvec3f t((float) -longSin[i],(float) longCos[i], 0);
         Cvec3f b = cross(n, t);

         *vtxIter = GenericVertex(
            x * radius, y * radius, z * radius,
            x, y, z,
            (float) 1.0 / slices*i, (float) 1.0 / stacks*j,
            t[0], t[1], t[2],
            b[0], b[1], b[2]);
         ++vtxIter;
...
void makeCylinder(int slices, float radius, float height, VtxOutIter vtxIter, IdxOutIter idxIter)
{
   ...
   float angle = (float) (2 * CS175_PI / slices);
   ...
   for (int i = 0; i < slices; ++i)
   {
      for (int j = 0; j < stacks; ++j)
      {
         ...
         *vtxIter = GenericVertex(
            x * radius, y, z * radius,
            x, y, z,
            (float) 1.0 / slices*i, (float) 1.0 / 2 * j,
            t[0], t[1], t[2],
            b[0], b[1], b[2]);
         ++vtxIter;
      }
...
{
   /*	PURPOSE:		Sets up index and vertex buffers for a triangle
      RECEIVES:
      RETURNS:
      REMARKS:
      */
...
{
   /*	PURPOSE:		Sets up index and vertex buffers for a point
      RECEIVES:
      RETURNS:
      REMARKS:
      */
...
{
   /*	PURPOSE:		Sets up index and vertex buffers for a line
   RECEIVES:
   RETURNS:
   REMARKS:
   */
